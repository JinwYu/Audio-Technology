...
void setup()
{
  fillSramBufferWithSinus(); // Fyll SRAM-bufferten
...
void loop()
{
  ...
  sampleFlag = 0;  // Sätt samplingsflaggan till false för att invänta nästa sample
  ...
  soundSampleFromADC = badc0 - 127; // Där badc0 är input, inkommande ljudsamplet.
  // Ta bort DC-offset. Tar minus med 127 för att korrigera dc-offset. Flyttar så signalen går runt till x-axeln.

  // Fixar så potentiometern ändrar volymen.
  soundSampleFromADC = soundSampleFromADC*(badc1/4 + 1); // Plussar med ett så inte volymen blir noll.

  // Overdrive
  // Hårdlimitera, vårt maxvärde är 128 och min. 
  // Fixar så signal över våra max och min-värden får distortion.
  if(soundSampleFromADC > 128) soundSampleFromADC = 128;
  if(soundSampleFromADC < -127) soundSampleFromADC = -127;
  
  // Lyfter upp ljudet till rätt DC-offset igen.
  sramBufferSampleValue = soundSampleFromADC + 127; 

  // Test av ljud loopback
  OCR2A = sramBufferSampleValue; // Läs inkommande ljudsampel och skicka sampelvärdet till PWM-utgången.
  //---------------------------------------------------------------------------
  ...
  */
  ...
  // Lägger till fördröjning med potentiometern.
  // Skapar därmed flangern, eftersom flanger har originalsignalen plus en fördröjd originalsignal
  // i vårt fall tas värdet på fördröjningen med potentiometern.

  // Börjar indexet på 0 igen när går till 512;
  bufferIndex = bufferIndex % 512;

  // "&" är bitwise.
  bufferIndex2 = bufferIndex2 & 511;

  // Summera signalerna och se till så signalen inte blir för hög genom att dela med 2.
  sramBufferSampleValue = (soundSampleFromADC + soundSampleFromSramBuffer) /  2; 

  // Skickar till utgången för att testa ljudet.
...
// Funktion för att fylla SRAM buffern med sinus
void fillSramBufferWithSinus(){
  float x = 0;
  float dx = 2 * M_PI / sizeof(sramBuffer); // Delta av x
  for (soundSampleFromSramBuffer = 0; soundSampleFromSramBuffer < sizeof(sramBuffer); soundSampleFromSramBuffer++) {
    float sinusSample = 127 * sin(x) + 127; 
    x = x + dx;
    sramBuffer[soundSampleFromSramBuffer] = sinusSample;
  }
}
